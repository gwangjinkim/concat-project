...
- Optional `-r/--recursive` behavior when `--input` matches folders
...
| `-r`, `--recursive`        | If an `--input` selector matches a folder, include files inside it recursively |
...
- **Folder recursion:** `--input src --recursive`
